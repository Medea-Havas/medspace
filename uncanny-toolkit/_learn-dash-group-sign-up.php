...
<?php
    ...
    public static function uncanny_group_signup_show_error_messages()
    {
        if ($codes = self::uncanny_group_signup_errors()->get_error_codes()) {
            echo '<div id="errors" class="uncanny_group_signup_errors">';
    ...
    public static function uncanny_group_signup_add_new_member()
    {
        ...
        if (isset($_POST['uncanny_group_signup_user_login']) && wp_verify_nonce($_POST['uncanny_group_signup_register_nonce'], 'uncanny_group_signup-register-nonce')) {
            ...
            $user_email   = sanitize_email(mb_strtolower($_POST['uncanny_group_signup_user_email']));
            $user_first   = sanitize_text_field(ucwords(mb_strtolower($_POST['uncanny_group_signup_user_first'])));
            $user_last    = sanitize_text_field(ucwords(mb_strtolower($_POST['uncanny_group_signup_user_last'])));
            $phone   = $_POST['uncanny_group_signup_user_phone'];
            $dni    = sanitize_text_field(strtoupper($_POST['uncanny_group_signup_user_dni']));
            $workplace    = sanitize_text_field(ucwords(mb_strtolower($_POST['uncanny_group_signup_user_workplace'])));
            $specialty    = sanitize_text_field(ucwords(mb_strtolower($_POST['uncanny_group_signup_user_specialty'])));
            $student    = $_POST['uncanny_group_signup_user_student'];
            $province    = $_POST['uncanny_group_signup_user_province'];
            $colnum    = $_POST['uncanny_group_signup_user_colnum'];
            $user_pass    = $_POST['uncanny_group_signup_user_pass'];
            $pass_confirm = $_POST['uncanny_group_signup_user_pass_confirm'];
            $legal = $_POST['uncanny_group_signup_user_legal'];
            $user_createdDate = date('Y-m-d H:i:s');
            $user_createdIP = $_SERVER['REMOTE_ADDR'];
            ...
            if ($user_first == '') {
                self::uncanny_group_signup_errors()->add('user_first_empty', 'Se requiere el nombre');
            }
            if ($user_last == '') {
                self::uncanny_group_signup_errors()->add('user_last_empty', 'Se requieren los apellidos');
            }
            if ($workplace == '') {
                self::uncanny_group_signup_errors()->add('workplace_empty', 'Se requiere el lugar de trabajo');
            }
            if ($specialty == '') {
                self::uncanny_group_signup_errors()->add('specialty_empty', 'Se requiere la especialidad');
            }
            if ($student == 'No') {
                if ($province == '') {
                    self::uncanny_group_signup_errors()->add('province_empty', 'Se requiere la provincia donde está colegiado');
                }
                if ($colnum == '') {
                    self::uncanny_group_signup_errors()->add('colnum_empty', 'Se requiere el número de colegiado');
                } else if (strlen((string) $colnum) < 5 || strlen((string) $colnum) > 5) {
                    self::uncanny_group_signup_errors()->add('colnum_format', 'El número de colegiado debe tener 5 cifras');
                }
            }
            if (strlen((string) $phone) < 9) {
                self::uncanny_group_signup_errors()->add('phone_number_format', 'El número de teléfono tiene menos de 6 cifras');
            }
            if (!validDni::check($dni)) {
                self::uncanny_group_signup_errors()->add('invalid_dni', 'El DNI/NIE no tiene formato correcto');
            }
            if ($legal != 'on') {
                self::uncanny_group_signup_errors()->add('invalid_legal', 'Para continuar es necesario aceptar el aviso legal y la política de privacidad');
            }
            ...
            if (empty($errors)) {
                $new_user_id = wp_insert_user(
                    array(
                        ...
                        'createdip' => $user_createdIP,
                        'role'            => get_option('uo_groups_default_user_role', get_option('default_role', 'subscriber')),
                    )
                );
                if ($new_user_id) {
                    // Brave update
                    update_user_meta($new_user_id, 'dni', $dni);
                    update_user_meta($new_user_id, 'phone', $phone);
                    update_user_meta($new_user_id, 'workplace', $workplace);
                    update_user_meta($new_user_id, 'specialty', $specialty);
                    update_user_meta($new_user_id, 'student', $student);
                    update_user_meta($new_user_id, 'province', $province);
                    update_user_meta($new_user_id, 'colnum', $colnum);
                    update_user_meta($new_user_id, 'createddate', $user_createdDate);
                    update_user_meta($new_user_id, 'createdip', $user_createdIP);
    ...
    <tr>
        <td>
            <input type="text" class="widefat" name="uncanny_group_organization_name"
                ...
                placeholder="Organization Name" /><br /> <span class="description">(optional)</span>
            <hr />
        </td>
    </tr>
    <tr>
        <td>
            <input type="text" class="widefat" name="uncanny_group_organization_contact"
                ...
                placeholder="Organization Contact" /><br /> <span class="description">(optional)</span>
            <hr />
        </td>
    </tr>
    <tr>
        <td>
            <input type="email" class="widefat" name="uncanny_group_organization_email"
                ...
                placeholder="Organization Email" /><br /> <span class="description">(optional)</span>
            <hr />
        </td>
    </tr>
    <tr>
        <td>
            <input type="tel" class="widefat" name="uncanny_group_organization_phone"
                ...
                placeholder="Organization Phone" /><br /> <span class="description">(optional)</span>
        </td>
    </tr>
</table>
<?php
    ...
    public static function render_signup_url_row_action($actions, $post)
    {
        if ("groups" === (string) $post->post_type) {
            if (current_user_can('manage_options')) {
                $actions['ulgm_signup_url'] = sprintf(
                    '<a href="%s">%s</a>',
...
class validDni
{
    public static function check($value)
    {
        ...
        if (preg_match($pattern, $dni)) {
            ...
            $letter = substr('TRWAGMYFPDXBNJZSQVHLCKET', $start, 1);
            if ($letter != $dni) {
                // Wrong ID, the letter of the NIF does not correspond
                return false;
            } else {
                // Correct ID'
                return true;
